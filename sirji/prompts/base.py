...
class PromptGeneratorBase(ABC):
  ...
  def can_interact_with_other_agents_prompt(self):
    ...
    if self.interact_with():
      can_interact_with_prompt += f"{self.short_name()} can interact with:\n"
  ...
  def other_agents_interaction_message_prompt(self):
      ...
      if self.interact_with(): 
        ...
        for instance in self.interact_with():
            interaction_message_prompt += f"The allowed messages from {self.short_name()} to {instance.short_name()} are:\n\n"
            ...
            for message_instance in incoming_message_instances:
                ...
                incoming_message_display_count += 1
            interaction_message_prompt += f"The allowed messages from {instance.short_name()} to {self.short_name()} are:\n\n"
  ...
  def caller_agent_interaction_message_prompt(self):
      ...
      if self.incoming_message_instances() or self.outgoing_message_instances():
        interaction_message_prompt += "Allowed message formats are described below.\n\n"   
        interaction_message_prompt += f"The allowed messages from {self.caller_short_name} to {self.short_name()} are:\n\n"
        incoming_message_display_count = 1
        ...
        for message_instance in incoming_message_instances:
            ...
            incoming_message_display_count += 1
        interaction_message_prompt += f"The allowed messages from {self.short_name()} to {self.caller_short_name} are:\n\n"
        outgoing_message_display_count = 1
